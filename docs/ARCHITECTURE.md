...
Sign-in uses Auth.js (NextAuth) with the Google provider, because the owner prefers his Google login. On successful Google sign-in, the backend checks the account email against an allowlist held in an environment variable (`ALLOWED_EMAILS`). Only an allowlisted account gets a session. Everyone else sees the sign-in screen and reaches no protected function.
